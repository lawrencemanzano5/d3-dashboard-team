...
var hasRenderedExpandedView = false;
...
function onButtonClick(e) {
    ...
    if (button.innerText === "Expand") {
        ...
        d3.select("#expand_div").style("display", "grid");

        if (!hasRenderedExpandedView) {
...
function addStarPlot(data, index) {
    ...
    var svg = grabSvgDimensions("vis1_expand" + (index + 1));
    console.log(svg);
...
function drawStarPlot(svg, data, height, width, top_margin, color_index, isOriginal) {
    ...
    let endRange = isOriginal ? height * 0.35 : height * 0.24;
    ...
    if(isOriginal) {
        ...
        addSubtitle(svg, height, width, evolCategories[data[0]["Evolution"]]);
    }
}

function addSubtitle(svg, svgHeight, svgWidth, subtitle) {
    svg.append("text")
        .attr("x", svgWidth / 2)
        .attr("y", svgHeight)
        .style("text-anchor", "middle")
